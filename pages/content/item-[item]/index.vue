  ...
  <div class="flex h-full w-full divide-x divide-zinc-600 text-white">
    ...
    >
      <h1 class="p-2 text-xs border-b border-zinc-600 text-zinc-500">
        content list - topic
        {{ store.list.data[itemIndex].name }}
      </h1>
      <div
      ...
      >
        <draggable
          v-if="store.list.data[itemIndex].items.length > 0"
          v-model="store.list.data[itemIndex].items"
          @start="drag = true"
          @end="drag = false"
          item-key="id"
            ...
            <div class="flex flex-row w-full min-h-[54px] divide-x divide-zinc-800">
              ...
              >
                <div class="px-3 pt-1 pb-2">
                  <label class="text-xs text-zinc-400 leading-tight">Item name:</label>
                  <input type="text" v-model="element.name" class="w-full cursor-text" />
                </div>
                <div class="px-3 pt-1 pb-2">
                  <label class="text-xs text-zinc-400 leading-tight">Item Link:</label>
              ...
              <div class="flex flex-col divide-y divide-zinc-800">
                ...
                />
                <i
                  class="min-w-[42px] h-full mdi mdi-arrow-right-bold flex justify-center items-center cursor-pointer text-blue-500 text-lg"
                  @click="$router.push(`${route.fullPath}/subitem-${element.id}`)"
                />
              </div>
            </div>
          </template>
        ...
        <div v-else class="h-full flex flex-col justify-center items-center">
          <div class="text-zinc-400">
            content list - topic
            {{ store.list.data[itemIndex].name }} is empty
          </div>
...
const itemIndex = store.list.data.findIndex(
  (item) => item.id == Number(route.params.item)
);

const addItem = () => {
  store.list.data[itemIndex].items.push({
    id: store.list.data[itemIndex].items.length,
    name: "",
    link: "",
    description: "",
    items: [],
...
const removeItem = (id: number) => {
  store.list.data[itemIndex].items = store.list.data[itemIndex].items.filter((item) => item.id != id);
};
